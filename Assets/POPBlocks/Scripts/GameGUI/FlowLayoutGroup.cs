...
namespace POPBlocks.Scripts.GameGUI
{
	...
	public class FlowLayoutGroup : LayoutGroup
	{
		...
		public float SetLayout(int axis, bool layoutInput)
		{
			...
			if (!layoutInput && rectChildren.Count > 1) {
				if (startAxis == Axis.Horizontal) {
			...
			{
				if (!layoutInput) {
					float newOffsetH = CalculateRowVerticalOffset(groupHeight, offset, currentBarSpace);
					float newOffsetV = CalculateColHorizontalOffset(groupWidth, offset, currentBarSpace);
					currentBarSize -= spacingBetweenElements;
					LayoutRow(_itemList, currentBarSize, currentBarSpace, workingSize, padding.left, newOffsetH, 0);
					LayoutCol(_itemList, currentBarSpace, currentBarSize, workingSize, newOffsetV, padding.top, 1);
				}
			}
